...
import { activeConference, computeRisks } from '../lib/metrics'
  ...
  return (
    <>
      ...
      <div className="grid g4">
        ...
        <StatTile label="영향 연사" value={new Set(risks.map(r => r.speakerId)).size} unit="명" foot={`총 ${state.speakers.filter(s => s.conferenceId === conf.id).length}명 중`} />
      </div>
      ...
      <Card title="판정 기준" sub="설정 &gt; 리스크 규칙에서 임계값을 조정합니다">
        <ul className="small sec" style={{ paddingLeft: 18, margin: 0, lineHeight: 1.8 }}>
          <li><strong>비자</strong> — 잔여일 &lt; (사증 심사 리드타임 + 사증발급인정서 심사 20일 + 버퍼 7일) → 즉시 조치</li>
